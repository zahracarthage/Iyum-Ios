...
/** `true` if we inside polygon, `false` otherwise */
@property (nonatomic, readonly, getter=isInside) BOOL inside;

/**
 * Distances to particular entrances, sorted by probability. Might be empty
 * in the case when the polygon is very large and no entry could be
 ...
 * the casee when the polygon is very large and no exit could be detected
