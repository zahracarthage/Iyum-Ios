 ...
 * If edge with given edgeId is not accessible, returns null
 */
- (nullable NSArray<CLLocation *> *)getEdgeShapeForEdgeId:(uint64_t)edgeId __attribute((ns_returns_retained));
/**
 * Returns geometry of given path on graph.
 * If any of path edges is not accessible, returns null.
 */
- (nullable NSArray<CLLocation *> *)getPathShapeForPath:(nonnull MBNNGraphPath *)path __attribute((ns_returns_retained));
/**
 * Returns geographical coordinate of given position on graph
 * If position's edge is not accessible, returns null.
 */
- (nullable CLLocation *)getPositionCoordinateForPosition:(nonnull MBNNGraphPosition *)position __attribute((ns_returns_retained));
